...
class Board:
    ...
    def __init__(
        ...
        rows: int,
        cols: int,
        max_ship_lenght: int = 4,
        seed: int | None = None,
    ):
        self.rng = random.Random()
        self.rng.seed(seed)

        self.max_ship_lenght: int = max_ship_lenght
        self.rows: int = rows
        self.cols: int = cols

        self.ships: list[Ship] = []
        self.num_ships_alive: int = 0
        self.ships_grid: Grid[Ship | None] = Grid(rows, cols, None)
        self.hit_grid: Grid[bool] = Grid(rows, cols, False)

        self.free_cells: dict[int, list[tuple[Pos, Pos]]] = (
            Board.generate_all_valid_ship_placements(
                rows, cols, max_ship_lenght, self.rng
            )
        )
    ...
    def check_hit(self, pos: Pos) -> HitResult:
        ...
        if not ship.alive:
            return HitResult.ship_was_destroyed
        ship.cells[pos.as_tuple] = False
        if ship.alive:
            return HitResult.ship_hit
        self.num_ships_alive -= 1
